...
library(ggplot2)
library(dplyr)
df <- read.table('td_3/VIN_QUALITE.txt', header = T)
```

```{r}
MANOVA <- function(X, Y) {
  observations <- split(X, Y) # on sépare les données par groupe
  # X ce sont nos variables et Y nos classes
  Moyennes <- lapply(observations, function(x) sapply(x, mean)) # on calcule les moyennes de chaque groupe
  Moyenne_globale <- sapply(X, mean)
  #les dimensions
  col <- ncol(X) #nombre de variables
  lignes <- nrow(X) #nombre d'observations
  
  # dispersion_tot --> variation totale des données
  #==> pour mesurer la dispersion totale des données autour de la moyenne globale : A quel point les données sont éloignées de la moyenne globale
  dispersion_tot <- (lignes - 1) * cov(X)
  
  # dispersion_intra_grp --> variation À L'intérieur de chaque groupe
  dispersion_intra_grp <- matrix(0, col, col)
  for(i in 1:length(observations)) {
    dispersion_intra_grp <- dispersion_intra_grp + (nrow(observations[[i]]) - 1) * cov(observations[[i]])
  }
  
  # dispersion_inter ==> variation entre les groupes
  dispersion_inter <- dispersion_tot - dispersion_intra_grp
  
  return(list(dispersion_tot = dispersion_tot, dispersion_intra_grp = dispersion_intra_grp, dispersion_inter = dispersion_inter))
}
```
...
X <- df[, -ncol(df)]  # Variables prédictives sauf la dernière
Y <- df[, ncol(df)]   # Variable à prédire == la dernière colonne
Resultat <- MANOVA(X, Y)
...
knitr::include_graphics('td_3/AFD_Axe_Facto.jpg')
```
...
```{r}
div_mat <- Resultat$dispersion_inter %*% solve(Resultat$dispersion_intra_grp)
div_mat
```
...
```{r}
diago <- eigen(div_mat)
D <- Re(diago$vectors)  # avec Re, on garde juste la partie réelle
D
```
...
norm_mat <- sqrt(diag(t(D) %*% Resultat$dispersion_intra_grp %*% D))
norm_mat
```
...
```{r}
Vect_normalise <- sweep(D, 2, norm_mat, "/")
Vect_normalise
```
...
```{r}
sans_partie_im <- Re(diago$values)
sans_partie_im
``` 
...
```{r}
disp <- sans_partie_im / sum(sans_partie_im)
disp
```
...
```{r, echo = F}
centree <- scale(X, center = TRUE, scale = FALSE) # on centre les données
```


```{r, echo = F}
Scores <- centree %*% Vect_normalise
Scores_df <- data.frame(Scores[, 1:2], Class = Y) # on garde que les deux premiers axes
colnames(Scores_df) <- c("Axe1", "Axe2", "Class") # on nomme les colonnes
head(Scores_df)
```
...
AFD_graph1 <- function(Scores_df) {
  centroids <- Scores_df %>%
    group_by(Class) %>%
    summarise(Axe1 = mean(Axe1), Axe2 = mean(Axe2))
  
  ggplot(Scores_df, aes(x = Axe1, y = Axe2, color = as.factor(Class))) +
    geom_point(size = 2) +
    geom_point(data = centroids, aes(x = Axe1, y = Axe2), 
               shape = 23, size = 4, fill = "white") +
    labs(x = "Axe 1", y = "Axe 2", color = "Classe") +
    theme_minimal()
}
...
```{r}
n <- nrow(X) # nombre d'observations total
p <- ncol(X) #nombre de variables
k <- length(unique(Y)) #nombre de classes
axe_selected <- 2

Wilks <- prod(1 - eta2[1:axe_selected])
chi2_stat <- -(n - 1 - (p + k)/2) * log(Wilks)
df <- p * (k - 1)
p_value <- pchisq(chi2_stat, df, lower.tail = FALSE)

resultat_wilks <- data.frame(
  Lambda_Wilks = Wilks,
  Chi2 = chi2_stat,
  ddl = df,
  P_value = p_value
...
```{r}
AFD <- function(X, Y, dispersion_tot, dispersion_intra_grp, dispersion_inter, nb_axes = 2) {
  # Matrice diago
  diago <- dispersion_inter %*% solve(dispersion_intra_grp)
  
  # Diagonalisation
  eigen_res <- eigen(diago)
  D <- Re(eigen_res$vectors)
  sans_partie_im <- Re(eigen_res$values)
  
  # Normalisation
  norm_factor <- sqrt(diag(t(D) %*% dispersion_intra_grp %*% D))
  Vect_normalise <- sweep(D, 2, norm_factor, "/")
  
  # Scores
  Z <- scale(X, center = TRUE, scale = FALSE)
  Scores <- Z %*% Vect_normalise
  Scores_df <- data.frame(Scores[, 1:nb_axes], Class = Y)
  colnames(Scores_df)[1:nb_axes] <- paste0("Axe", 1:nb_axes)
  
  # Test de Wilks
  eta2 <- sans_partie_im / (1 + sans_partie_im)
  n <- nrow(X)
  p <- ncol(X)
  k <- length(unique(Y))
  
  Wilks <- prod(1 - eta2[1:nb_axes])
  chi2_stat <- -(n - 1 - (p + k)/2) * log(Wilks)
  df <- p * (k - 1)
  p_value <- pchisq(chi2_stat, df, lower.tail = FALSE)
  ...
  return(list(
    Loading_factors = Vect_normalise,
    Valeurs_propres = sans_partie_im,
    Inertie = disp,
    Scores = Scores_df,
    Test_Wilks = resultat_wilks
  ))
}
...
```{r, echo = T}
Resultat_AFD <- AFD(X, Y, Resultat$dispersion_tot, Resultat$dispersion_intra, Resultat$dispersion_inter, nb_axes = 2)
AFD_graph1(Resultat_AFD$Scores)
...
X_iris <- iris[, 1:4]
Y_iris <- iris[, 5]
Resultat_iris <- MANOVA(X_iris, Y_iris)
Resultat_AFD_iris <- AFD(X_iris, Y_iris, Resultat_iris$dispersion_tot, Resultat_iris$dispersion_intra, Resultat_iris$dispersion_inter, nb_axes = 2)
...
```{r}
Resultat_AFD_iris <- AFD(X_iris, Y_iris, 
                    Resultat_iris$dispersion_tot, 
                    Resultat_iris$dispersion_intra, 
                    Resultat_iris$dispersion_inter, 
                    nb_axes = 2)
Resultat_AFD_iris$Loading_factors
```
...
```{r}
Resultat_AFD_iris$Valeurs_propres
```
