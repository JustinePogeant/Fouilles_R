...
```{r, echo = T}
df <- data.frame(
  Salaire  = c('< 30', '30 - 50', '30 - 50', '30 - 50', '> 50'),
...
```{r, echo = T}
X <- df[, c("Salaire", "Impots", "Etudiant")]
...
Dans un premier temps Nous cherchons ici à créer les tableaux de contingence entre les différentes variables du prédicteur et la variable à prédire. 
...
prop_cond <- lapply(contingence, function(x){
  tab_corr <- x + m
  denom <- colSums(x) + m * k
  sweep(tab_corr, 2, denom, FUN = "/")
})

names(prop_cond) <- names(X)
...
```{r, echo = T}
affectation <- matrix(0,
                         nrow = nrow(data_test),
                         ncol = length(categories))
colnames(affectation) <- categories

for(i_test in 1:nrow(data_test)){
  
  lnV <- NULL
  for(ic in 1:length(categories)){
    somme <- 0
    for(i in 1:ncol(X)){
      ar <- prop_cond[[i]]
      somme <- somme + log( ar[categories[ic], data_test[i_test, i]] )

    }
    lnV <- c(lnV, log(priori[ic]) + somme)
  }
  
  affectation[i_test, ] <- lnV
}
...
Naive_Bayes <- function(X, Y, m = 1){
  ...
  # tableau des a priori
  priori <- prop.table(table(Y))
  
  # construction des tableaux de contingence
  contingence <- lapply(X, function(v){
    table(Y, v)
  })
  
  k <- length(unique(Y))
  
  # correction + probas conditionnelles
  prob_cond <- lapply(contingence, function(tab){
    tab_corr <- tab + m
    denom <- colSums(tab) + m * k
    sweep(tab_corr, 2, denom, FUN = "/")
  })
  
  names(prob_cond) <- names(X)
...
```{r, echo = T}
Predict_Bayes <- function(prob_cond, priori, X_pred){
  
  categories <- names(priori)
  log_post <- matrix(0, nrow = nrow(X_pred), ncol = length(categories))
  colnames(log_post) <- categories
  
  for(i_test in 1:nrow(X_pred)){
    ...
    for(ic in 1:length(categories)){
      ...
      for(i in 1:ncol(X_pred)){
        ar <- prob_cond[[i]]
        classe <- categories[ic]
        modal  <- X_pred[i_test, i]
        somme  <- somme + log(ar[classe, modal])
      }
  ...
  prediction <- apply(log_post, 1, function(x){
    names(x)[which.max(x)]
  })
  
  out <- data.frame(log_post, Pred = prediction)
  return(out)
}
```
...
```{r}
resultat_pred <- Predict_Bayes(res_nb$prob_cond, res_nb$priori, X)
resultat_pred
```
...
```{r}
prediction <- Predict_Bayes(resultat$prob_cond, resultat$priori, X)
table(prediction$Pred, y)
```
...
model <- naiveBayes(event ~ ., data = data_test)
prediction2 <- predict(model, data_test)

confusionMatrix(prediction2, data_test$event)
```
